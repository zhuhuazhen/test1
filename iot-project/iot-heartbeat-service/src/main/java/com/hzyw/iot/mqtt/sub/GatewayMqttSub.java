...
public class GatewayMqttSub extends AbstractSubHandler  {
	...
	public GatewayMqttSub(MqttCallbackImpl mqttCallbackImpl,MqttConnectionConfig mqttConnectionConfig){
	...
	public void subscribe() {
		try {
			...
			this.getOptions().setWill("heartbeatTopic", "heartbeat监听线程掉线!".getBytes(), this.getQos(), false);
			// 设置断开后重新连接
	...
	public Map<String, String> getConfig() {
		...
		config.put("topic", connectionConfig.getServiceOffline().get("topic"));
		config.put("qos", connectionConfig.getServiceOffline().get("qos"));
		...
		config.put("clientId", connectionConfig.getServiceOffline().get("clientId"));
		return config;
