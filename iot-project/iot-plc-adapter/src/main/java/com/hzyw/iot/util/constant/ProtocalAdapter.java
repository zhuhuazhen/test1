...
public class ProtocalAdapter{
     ...
     * @return
     */
    public static synchronized String  generaMessageReq(String uuid,String code, String cmd, String paramBody) throws Exception {
        ...
        paramBody=O_CODE_VAL.PDTTemplate(codes,cmd,paramBody,null);
    ...
    public static synchronized String  generaMessageRes(String uuid,String code, String cmd, String paramBody,ChannelHandlerContext ctx) throws Exception {
        log.info("==========generaMessageRes方法==入参 指令码:"+cmd+", 控制码:"+code);
        if(StringUtils.trimToNull(code)==null || StringUtils.trimToNull(cmd)==null)
            throw new Exception("=====generaMessageRes方法===按指令生成对应的协议码报文时,指令:"+cmd+"或 控制码:"+code+" 入参为空!");
        //设备ID (校验16进制的长度6个字节)
        HEAD_TEMPLATE.setUID(checkDeviceUID(uuid));
        ...
        log.info("=======cmd:"+cmd+",===code:"+code+"====根据指令调PDT模板 生成[指令参数]的十六进制值:"+paramBody);
        if(("80H".equals(code) || "04H".equals(code))){
            //上报或响应过来的，若无需返回的直接返回空串(paramBody为空:表示不需响应)
            if("".equals(paramBody)) return ""; else code="80H";
        }
        HEAD_TEMPLATE.setPDT(paramBody);
        ...
        log.trace("====计算校验码(CS)时 传参的控制码: "+code);
        //System.out.println("====计算校验码(CS)时 传参的控制码: "+codes);
        //计算校验码(CS): 从“帧起始符”到校验码之前的所有字节的模256的和，即各字节二进制算术和，不计超过256 的溢出值
        HEAD_TEMPLATE.setCS(CLAC_CS_SUM(C_CODE_VAL.CValueMethod(code),L_SIZE,O_CODE_VAL.CmdValueMethod(cmd),paramBody));
        //根据指令获取相应请求或响应报文
        String resMessage=O_CODE_VAL.OrderMethod(cmd,code);
        log.info("====指令码:"+cmd+",===根据指令获取相应 组装后的请求协议报文(resMessage):"+resMessage);
        log.info("====PLC地址(uid):"+HEAD_TEMPLATE.getUID());

        //校验生成的协议报文是否有错误
        byte[] resp=ConverUtil.hexStrToByteArr(resMessage);
        boolean validateRes=ValidateProtocalMessage(resp);
        if (!validateRes) log.error("设备->主机 的 协议报文的格式或校验有错误! 无法生成报文!");
        return resMessage;
    }
    ...
    public static String  messageRequest(JSONObject jsonObject){
        ...
        try {
            ...
            if(!isReqBlock(outList,algorithm_uuid,cmd,nodeID,mesgID,uuid)){
                return generaMessageReq(uuid,code,cmd,pdtParamsStr);
            }
        } catch (Exception e) {
    ...
    private static boolean isReqBlock(List<Map<String,Object>> outList, String... params){
        ...
        try {
            ...
            if(CacheReqID.containsKey(key)){
                ...
                if(time_out<=3 && time_out>0) {//调置 超时3秒, 等待3秒响应时间
    ...
    public static String  messageRespose(byte[] resp, ChannelHandlerContext ctx){
        ...
        try {
            ...
            return generaMessageRes(uuid,code,cmd,paramBody,ctx);
        } catch (Exception e) {
    ...
    public static String batchRequestCode(String code, String cmd, String pdt)throws Exception{
        ...
        return generaMessageReq(HEAD_TEMPLATE.getUID(),code,cmd,pdt);
    }
