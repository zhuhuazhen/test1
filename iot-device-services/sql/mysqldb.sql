...
(
    ATTR_KEY   VARCHAR(200) not null comment '属性键'
        primary key,
    ATTR_NAME  VARCHAR(200) comment '属性名',
    VALUE_TYPE VARCHAR(100) comment '数据类型',
    UNIT       VARCHAR(100) comment '单位',
    RATIO      int default 1 comment '单位/标准单位比率，正为乘数，负为商数',
    METADATA   VARCHAR(200) comment '属性元数据 (描述性元数据,JSON格式)',
    STD_ATTR_KEY VARCHAR(200) comment '映射标准大类模型表'
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8 comment ='设备属性表';
############################################################################

############          标准属性定义表                ######################
create table if not exists DEVICE_ATTRIBUTE_STD_T
(
    ATTR_KEY   VARCHAR(200) not null comment '属性键'
        primary key,
    ATTR_NAME  VARCHAR(200) comment '属性名',
  ...
  DEFAULT CHARSET = utf8 comment ='标准属性定义表';
############################################################################
