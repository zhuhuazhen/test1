...
import com.hzyw.iot.platform.util.json.DevInfoJsonData;
...
public class MetricSender {
    ...
    public void sendEquipmentInfoToMqtt(String deviceId,List<DevInfoJsonData> devInfos) {
        //上报所有数据
            Map map = new HashMap();
            for(DevInfoJsonData devInfoJsonData : devInfos){
                map.put("equipmentCode", devInfoJsonData.getUuid());
                map.put("sn", devInfoJsonData.getSn());
                map.put("equipmentName", devInfoJsonData.getDevice_type_name());
                map.put("equipmentType", devInfoJsonData.getDevice_type_code());
                map.put("manufacturerName", devInfoJsonData.getVendor_name());
                map.put("manufacturerCode", devInfoJsonData.getVendor_code());
                map.put("equipmentModel", devInfoJsonData.getModel());
                map.put("productionDate", devInfoJsonData.getDate_of_production());
                map.put("softwareSpecification", devInfoJsonData.getVersion_software());
                map.put("hardwareSpecification", devInfoJsonData.getVersion_hardware());
                map.put("guaranteePeriod", devInfoJsonData.getUp_time());
                map.put("equipmentIp", devInfoJsonData.getIpaddr_v4());
                map.put("ipaddrV6", devInfoJsonData.getIpaddr_v6());
                map.put("equipmentMac", devInfoJsonData.getMac_addr());
                map.put("malfunction", devInfoJsonData.getMalfunction());
            }
            //查询获取设备信息
            Equipment deviceInfoDO = equipmentCache.get(deviceId);
            map.put("deviceId", deviceId);
            ...
            String jsonString = JSON.toJSONString(map);
            //组装主题
            String topicName = "EquipmentInfo/" + deviceInfoDO.getEquipmentType().getDomainFlag();
            //发送MQTT消息给应用平台
            mqttGateway.sendToMqtt(topicName, jsonString);
            log.info("sendDevAttrToMqtt(devInfos), topic = {}, msg = {}", topicName, jsonString);
    }
